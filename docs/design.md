...
| **Known gap:** `.claude.json` stays visible. It lists every project path and the account email, and it is written live by the agent, so filtering it risks breaking Claude Code for a leak that is paths rather than project content. `telemetry/` likewise: no project paths were found in it. Both are deliberate caps, not oversights. The file is bound **inside** the state directory, at `~/.claude/.claude.json`, with `CLAUDE_CONFIG_DIR` pointing Claude Code there: Claude Code writes it through a lock directory and a temp file created beside it, then a rename, and beside the read-only `$HOME` those failed silently, so before this bind every write a sandboxed session made to it (folder trust, per-project allowed tools and MCP servers, app state) was lost while the session reported success. The rename onto the bind mount fails and Claude Code rewrites the file in place; that write is not atomic. | `profiles/claude.sh` (`profile_config_binds` with `SRC<TAB>DEST`, `profile_env_set`, `profile_env_refuse`); the engine removes the empty mount-point file bwrap leaves on the host. | `tests/unit/argv.bats`, `tests/unit/profile-claude.bats` (the bind, the variable, the mount-point cleanup), `tests/integration/config-file.bats` (the writer's steps from inside; the write reaches the host). |
