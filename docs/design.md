...
| Secret stores never enter. `~/.ssh`, `~/.gnupg`, `~/.aws`, `~/.config/{gcloud,sops}`, `~/.kube`, `~/.docker`, `~/.password-store` are refused as CWD and as `AGENT_SANDBOX_RO`/`_RW`. `/`, `$HOME` and any parent of `$HOME` are refused too. | `_as_check_paths`, CWD checks. | `tests/unit/helpers.bats` (`_as_check_paths`), `tests/unit/argv.bats` (CWD and bind refusals: exit 1, bwrap never invoked). |
...
| Per-project policy is trust-gated. A project's `.agent-sandbox` (allow hosts, memory scoping) is honored only after `--trust` records its SHA-256; an edit by anyone, the agent included, drops it to untrusted and it is ignored until re-approved, so the agent can only invalidate trust, never grant it. The trust store and global config live under `~/.config/agent-sandbox`, which is never bound into the sandbox. | `_as_dotfile_trusted`, `_as_trust_record`, `_as_dotfile_parse`, `_as_trust_review`. | `tests/unit/dotfile.bats` (unapproved ignored with a pointer to `--trust`, an edit re-blocks, an approved file adds allow hosts, `--trust` records the hash and offers to git-ignore). |
