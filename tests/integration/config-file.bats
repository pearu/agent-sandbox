...
PROBE
  ...
  printf '{"seed":1}' >"$IHOME/.claude.json"
}
...
@test "inside: the config file is at ~/.claude/.claude.json with the host's content and CLAUDE_CONFIG_DIR points there; lock and temp file work beside it; the rename fails on the mount point; the in-place write reaches the host" {
  run_claude
  [ "$status" -eq 0 ]
  ...
  [ "${M[inplace_write]}" = yes ]
  # the host's file carries what was written inside
