...
| `--connect SPEC` | `AGENT_SANDBOX_CONNECT` | `[connect]` | per-channel `channel = mode [source] [scope]`: how much of your native install this project's sandbox sees, on the scale `own < copy < copy-on-write < read-only < read-write`; `;`-separated in the variable (the preset decides where each channel starts) ([connections.md](docs/connections.md)) |
