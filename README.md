...
| `--preset NAME` | `AGENT_SANDBOX_PRESET` | `[sandbox] preset` | where every channel sits before any `[connect]` override: `independent`, `default` (read your files live, keep the sandbox's writes to itself) or `shared` (one directory, both ways -- the engine before 0.3) (default) ([connections.md](docs/connections.md)) |
...
| `--user-mcp MODE` | `AGENT_SANDBOX_CLAUDE_USER_MCP` | `[claude] user-mcp` | whether the host's user-level MCP servers (`mcpServers` in `~/.claude.json`) reach this project's sandbox: `inherit` carries them into the project's copy of the config file and refreshes them each launch, `none` leaves them out, closing the one MCP channel that copy keeps open (inherit) ([config.md](docs/config.md)) |
