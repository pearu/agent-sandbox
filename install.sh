...
if [[ -f "$ca_in_store" ]]; then
  ...
  ca_src=""
  for cand in "$HOME/.mitmproxy/mitmproxy-ca-cert.pem" "$HOME/.mitmproxy/mitmproxy-ca-cert.cer"; do
    [[ -f "$cand" ]] && {
      ca_src="$cand"
      break
    }
  done
  if [[ -z "$ca_src" ]] && ((DRY_RUN)); then
    info "(dry-run) would generate the mitmproxy CA and trust it system-wide"
  elif [[ -z "$ca_src" ]]; then
  ...
  if [[ -n "$ca_src" ]]; then
    ...
    run_sys sudo cp "$ca_src" "$ca_in_store"
    run_sys sudo update-ca-certificates
    ((DRY_RUN)) || ok "CA installed and trusted"
  fi
fi
