...
| `bg-daemon.sh` | host-side: does `claude --bg` through the launcher leave a supervisor, a roster entry or a socket dir behind, and is `~/.claude/daemon` empty inside; `--native` runs the unsandboxed control | yes |
