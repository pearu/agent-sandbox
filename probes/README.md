...
| `wrapper-tryout.sh` | host-side: try the engine's `--wrap` role live — points `CLAUDE_CODE_PROCESS_WRAPPER` at this checkout's engine, starts a background session, shows the `--bg-spare` worker running under bwrap, then cleans up | yes |
