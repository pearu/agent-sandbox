...
conn_setup part1-read-only

conn_cell R1 read-only "the source is readable inside"
...
conn_cell R2 read-only "a write inside fails, and the source is untouched"
tok="$(conn_source_write "$CONN_CHANNEL_FILE")"
...
conn_cell R3 read-only "a source change is visible at the next launch"
