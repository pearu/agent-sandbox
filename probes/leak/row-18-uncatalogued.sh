...
plant() {
  ...
  # -g: a bare `declare` inside a function is LOCAL, and the loop below needs the table
  declare -gA TOK
...
leak_cell t1-native
plant
FIRST="${PATHS[0]%%|*}" # after plant: the table is built with the cell
...
# ONE PATH, ONE EXPERIMENT. Each iteration builds its own tree: a path that reads as
# shared is shared on its own, not because the iteration before it warmed the config.
n=0
for entry in "${PATHS[@]}"; do
  rel="${entry%%|*}"
  docs="${entry#*|}"
  n=$((n + 1))
  cellname="t2-$(printf '%02d' "$n")-${rel%%/*}"
  ...
  leak_watch_stop
  cp "$LEAK_WATCH_OUT" "$LEAK_RUN/records/t2-$n.reads" 2>/dev/null || true
  leak_record "$cellname" --set "topology=T2" --set "net=none" \
    --set "sandboxed=yes" --set "path=$rel" --set "docs=$docs" \
    --set "canary=${TOK[$rel]}" --reader "$LEAK_RUN/t2-$n.json" \
    --set-file "reads=$LEAK_RUN/records/t2-$n.reads"
done
