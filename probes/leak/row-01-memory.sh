...
# docs/design.md and docs/config.md; not re-measured here). leak_cell makes A and B
...
#
# ONE CELL = ONE TREE. Every cell below calls leak_cell, which builds a new HOME, a new
# config and two new repositories under /tmp and moves them into the run directory when
# the cell ends. Nothing is shared between cells and nothing is cleaned between them:
# plant() rebuilds the substrate from nothing each time, so the canaries differ per cell
# and a verdict can only have come from the cell that recorded it.
#
...
# A's memory carries one token; B's own memory carries a DIFFERENT one. B's is the
# negative control: inside the sandbox B must still reach its own project. Without it,
# "A's memory is unreachable" cannot be told apart from "no config is visible in there
# at all", and both would read as isolation.
plant() {
  ...
  # The reader identifies the canary by CONTENT, not by path, and keeps the three
  # outcomes distinguishable (opened-and-found / opened-and-absent / could not open).
...
# write above changed the substrate under the cell that just measured it.
...
leak_record "t2-share-named" --set "topology=T2-share" --set "net=none" \
  ...
  --set "write_attempt=$SHARE_WRITE" --set "source_intact=$SHARE_INTACT" \
  --reader "$LEAK_RUN/t2-share.json"
leak_say "  write into the shared memory: $SHARE_WRITE (A's memory intact: $SHARE_INTACT)"
